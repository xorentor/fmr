...
#define LOWER_B(b) (b|0x20)

static INLINE void fmf_reverse( unsigned char s[] );
...
int32 fmf_strcasecmp( const char *string1, const char *string2 ) {
	while( LOWER_B(*string1) == LOWER_B(*string2) && *string1 && *string2 ) {
		string1++;
		string2++;
	}
...
uint32 fmf_strlen( const char *string ) 
{
...
uint32 fmf_strxfrm( char *dst, const char *src, uint32 n )
{
#ifdef LIBSTR_DEBUG
	dbgmsg( "strxfrm_default" );
#endif
	uint32 srclen, copysize;
...
uint32 fmf_strspn( const char *s, const char *accept )
{
#ifdef LIBSTR_DEBUG
...
void *fmf_memchr(const void *cs, int32 c, uint32 count)
{
#ifdef LIBSTR_DEBUG
...
#else
void *fmf_memchr(const void *s, int32 c, uint32 n)
{
#ifdef LIBSTR_DEBUG
...
int32 fmf_memcmp( const void *s1, const void *s2, uint32 len )
{
#ifdef LIBSTR_DEBUG
...
#else
int32 fmf_memcmp( const void *cs, const void *ct, uint32 count )
{
#ifdef LIBSTR_DEBUG
...
void *fmf_memmove(void *dest, const void *src, uint32 n)
{
#ifdef LIBSTR_DEBUG
	dbgmsg( "memmove_x86_32" );
       ...
       __asm__ __volatile__(
               ...
               /*
                * We gobble 16 chars forward in each loop.
                */
               "3:\n\t"
               "sub $0x10, %0\n\t"
               "mov 0*4(%1), %3\n\t"
                ...
                */
                ...
                /*
                 * We gobble 16 chars backward in each loop.
                 */
                "7:\n\t"
                "sub $0x10, %0\n\t"
                ...
                /*
                 * Move data from 8 chars to 15 chars.
                 */
                ".p2align 4\n\t"
                "1:\n\t"
                "cmp $8, %0\n\t"
                ...
                /*
                 * Move data from 4 chars to 7 chars.
                 */
                ".p2align 4\n\t"
                "8:\n\t"
                "cmp $4, %0\n\t"
                ...
                /*
                 * Move data from 2 chars to 3 chars.
                */
                ".p2align 4\n\t"
                "9:\n\t"
                "cmp $2, %0\n\t"
...
#else
void *fmf_memmove(void *dest, const void *src, uint32 count)
{
#ifdef LIBSTR_DEBUG
...
void *fmf_memset(void *s, int32 c, uint32 n)
{
#ifdef LIBSTR_DEBUG
...
#else
void *fmf_memset(void *s, int32 c, uint32 count)
{
#ifdef LIBSTR_DEBUG
...
uint32 fmf_strcspn( const char *s, const char *reject )
{
#ifdef LIBSTR_DEBUG
...
char *fmf_strncat(char *dest, const char *src, uint32 count)
{
#ifdef LIBSTR_DEBUG
...
int32 fmf_strncmp(const char *cs, const char *ct, uint32 count)
{
#ifdef LIBSTR_DEBUG
...
char *fmf_strncpy(char *dest, const char *src, uint32 count)
{
#ifdef LIBSTR_DEBUG
	dbgmsg( "strlen_x86_32" );
#endif
...
#else
char *fmf_strncpy( char *strDest, const char *strSource, uint32 count ) {
#ifdef LIBSTR_DEBUG
...
static INLINE void fmf_reverse(unsigned char s[])
{
     ...
     for (i = 0, j = fmf_strlen(s)-1; i<j; i++, j--) {
         c = s[i];
         s[i] = s[j];
         s[j] = c;
     }
...
int32 fmf_itoa( int32 num, unsigned char* str, int32 len, int32 base )
{
        int32 sum = num;
        int32 i = 0;
        int32 digit;
