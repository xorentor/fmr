...
static void *pool_create_block( pool_t *p, int32 size );
/*
 * public
 * release node
 */
...
pe_t *pool_getnode( pool_t *p )
{
...
#endif
		p->freenode = pool_create_block( p, p->blocks_number );
	} else {
	...
	e = p->freenode;
	fmf_memset( p->freenode->data, 0, p->offset );
...
static void *pool_create_block( pool_t *p, int32 size )
{
	...
	e = (pe_t *)malloc( sizeof( pe_t ) * size );
	elements_mem = malloc( size * p->offset ); 
 ...
 * Create pool
 */
pool_t *pool_create( const int32 offset, const int32 blocks )
{	
	pool_t *p;
 ...
 * Get pool by name
 */
pool_t *pool_get( const int32 name )
{
	pool_t *p;
